...
class GoogleDriveClient:
        ...
        for f in range(0, len(file_list)):
            try:
                if file_list[f].get("mimeType") == "audio/mpeg":
                    ...
                    fh = open(self.DOWNLOAD_FILE_DIR + file_list[f].get("name"), "wb")
                    ...
                    while done is False:
                        ...
                        if done:
                            self.print_finish_download(file_list[f].get("name"))
            except Exception as error:
                print(
                    f"""Audio File Download Error: 
                    {error}
                """
                )
    ...
    def replace_file(
        self, parent_folder_name, existing_file_id, file_name, file_path, file_type
    ):
        ...
        try:
            ...
            upload_request = (
                self.service.files()
                .update(media_body=media, fileId=existing_file_id, fields="id")
                .execute()
            )
