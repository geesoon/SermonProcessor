...
# step 1: Define the requirement (desktop app or web app?)
# step 2: Design the object relationship
# step 3: Design the interface / contract / entry point
# step 4: Rearrange the code
# step 5: Add test code
# step 6: Design UI layer (web?)
# step 7: Add the UI layer
    ...
    for index in df.index:
        if df.loc[index, "File_Name"] == row["File_Name"]:
            if dt["generate_video_start"] != None:
                df.loc[index, GEN_VIDEO_START_CN] = dt["generate_video_start"].strftime(
                    "%d/%m/%Y %H:%M:%S"
                )

            if dt["generate_video_end"] != None:
                df.loc[index, GEN_VIDEO_END_CN] = dt["generate_video_end"].strftime(
                    "%d/%m/%Y %H:%M:%S"
                )
...
def filter_not_uploaded_recording(df, folder_list):
    ...
    df = df.loc[
        (df[UPLOAD_START_CN] == "") | (df[UPLOAD_END_CN] == "") | (df["URL"] == "")
    ]
    for index, row in df.iterrows():
        f = get_file_from_folder_list(row["File_Name"], "audio/mpeg", folder_list)
        if f != None:
...
def main():
    ...
    try:
        ...
        audio_folder_list = google_drive_service.get_folder_list(audio_folder_id)
        audio_message_excel_file = get_file_from_folder_list(
            AUDIO_MESSAGE_LIST_FILE_NAME,
            AUDIO_MESSAGE_LIST_MIME_TYPE,
            audio_folder_list,
        )
        if audio_message_excel_file != None:
            ...
            df = pd.read_excel(
                "./" + AUDIO_MESSAGE_LIST_FILE_NAME + ".xlsx",
                header=0,
                keep_default_na=False,
            )

            not_video_generated_gd_file_list, not_video_generated_recording_df = (
                filter_not_video_generated_recording(df, audio_folder_list)
            )
            not_uploaded_recording_gd_file_list, not_uploaded_recording_df = (
                filter_not_uploaded_recording(df, audio_folder_list)
            )

            print_df("Recording without video", not_video_generated_recording_df)
            ...
            if len(not_uploaded_recording_df.index) == 0:
                print_process(
                    "Note",
                    "All sermon have ben uploaded, refer to the excel file for the URLs",
                )

            print_process(
                "3", "Download sermon recordings that have not generated its video file"
            )
            ...
            for index, row in not_video_generated_recording_df.iterrows():
                ...
                try:
                    video_spec, generate_video_start, generate_video_end = (
                        video_generator.generate_video_files(row)
                    )
                    print(video_spec)
                    df = update_audio_message_list_excel_file(
                        row,
                        df,
                        dict(
                            upload_start=None,
                ...
                try:
                    print(video_spec)
                    url, upload_start, upload_end = youtube_uploader.upload_video(
                        video_spec
                    )
